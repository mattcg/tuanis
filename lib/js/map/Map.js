...
Map.prototype.scale = function(brewer, stops, type) {
	var values, scale, paths, i, l, stats = this.data.stats;
...
Map.prototype.defs = function() {
	var svg = this.svg, pattern, defs, path, svgNs = 'http://www.w3.org/2000/svg';
	...
	if (!svg.getElementById('stripe')) {
		...
		pattern = defs.appendChild(document.createElementNS(svgNs, 'pattern'));
		pattern.id = 'stripe';
		...
		path = pattern.appendChild(document.createElementNS(svgNs, 'path'));
